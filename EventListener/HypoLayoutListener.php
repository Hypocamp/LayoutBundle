...
class HypoLayoutListener {
    ...
    TwigEngine $templating, LayoutTwigExtension $layout, ControllerListener $controllerListener, array $configuration, $interceptRedirects = false, $mode = self::ENABLED )
    {
...
//				|| !$response->headers->has('X-Debug-Token')
        ...
        $template = $this->getTemplateName(
            $this->controllerListener->layout, $controller_method, $this->configuration[ 'templates' ] );

        $blocs = $this->getBlocs(
            $this->controllerListener->blocs, $controller_method, $this->configuration[ 'blocs' ] );

        $params = array_merge(
            is_array( $paramsConfig ) ? $paramsConfig : array(), array( 'content' => $response->getContent() ), $this->layout->variables, $blocs
        );
